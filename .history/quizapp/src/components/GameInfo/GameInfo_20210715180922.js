...
function GameInfo() {
  ...
  let [timeDifficulty, setTimeDifficulty] = useState(
    timerOptions[0].value.multiplier
  );
  let [questionDifficultyState, setQuestionDifficulty] = useState(
    questionDifficulty[0].value.multiplier
  );
  let [multiplier, setMultiplier] = useState(1.0);

  useEffect(() => {
    console.log("first value: ", timeDifficulty);
    console.log("second value: ", questionDifficultyState);

    let tempMultiplier = (
      (parseFloat(timeDifficulty) * parseFloat(questionDifficultyState)) /
      2
    ).toFixed(2);
    console.log("tempMultiplier: ", tempMultiplier);
  ...
  useEffect(() => {
    ...
    console.log("tempMultiplier: ", tempMultiplier);
    setMultiplier(tempMultiplier);
  }, [questionDifficultyState]);

  function onSelect(e) {
    console.log("e:", e);
    if (e.value.type === "Time") {
      console.log("time: ", e.value.multiplier);
